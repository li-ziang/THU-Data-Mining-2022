...
既利用“用户-商家”pair的统计数据，也利用用户和商家个体的统计数据。使用data_format1，以“用户-商家”对作为样本构建特征，特征围绕用户个体、商家个体以及用户与商家交互信息构建。
...
该特征工程单独的代码位于feature1.py。其中`generate_data`利用data_format1构建特征，输出数据用于训练与测试；如果是首次运行的话，会在源文件所在目录下生成`.cache`目录存放数据处理结果，便于后续使用。
...
代码在`run.py`。`run.py`内使用`xgboost`库中的XGBClassifier，训练模型并输出测试结果。通过`python run.py --model xgboost`运行XGBoost代码。

run.py中整合了这两种特征工程以及前两种模型（XGBoost和MLP），使用者可以在run.py里自由组合任一种特征工程和模型。run.py里的preprocess1和preprocess2函数为两种不同的特征工程，run_mlp和run_xgb函数为运行两种不同模型的代码。evaluate函数被run_mlp函数调用，对模型在验证集上进行评估。
...
模型使用第一种特征工程时，验证集上的ROC-AUC可以达到0.717；在比赛平台上ROC-AUC为0.684637，为我们组最终提交成绩。
...
代码依然位于run.py。通过`python run.py --model mlp`运行MLP代码。
...
|         | 特征工程1 | 特征工程2 |
| ------- | --------- | --------- |
| XGBoost | 0.7173    | 0.6223    |
| MLP     | 0.4989    | 0.6183    |

第一种特征工程在XGBoost上表现很好但在MLP上表现很差。第二种特征工程在两个模型上表现差不多，都不算很好。

从模型比较的角度来说，XGBoost明显好于MLP。这在深度学习时代比较反常，因为一般来说深度学习的模型表现优于传统模型。估计出现这一现象是因为这一数据集的噪声较多，深度神经网络的参数量较大，很容易过拟合于噪声从而造成效果不好。同时深度学习的优势在于可以不用设计精细化的特征，直接从数据中学习，然而这一数据集本身很难直接学习。由于我们已经设计了精细化的特征，深度学习的这一优势荡然无存，传统机器学习方法反而更适合有精细化特征的学习。

从特征工程比较的角度来说，特征工程1很适合XGBoost却不适合MLP。不适合MLP估计是因为它结合了较多用户和商家各自的信息。这些信息显然不如“用户-商家”pair的统计数据与结果更直接紧密地相关，属于一些有用但不那么有用的额外信息。XGBoost可以从中获益，但对于深度学习来说，它们的大量出现冲淡了更有用的“用户-商家”pair统计信息，给了模型的梯度错误的引导，导致模型训练出来效果不佳。特征工程2不存在这一问题，因此在MLP上表现较好。然而由于特征工程2缺少用户和商家各自的统计信息这一有价值的信息，XGBoost的表现也就没那么好。



##### 年龄与性别的onehot信息

我们默认把性别和年龄转化为onehot。XGBoost模型也可以不适用onehot的性别和年龄信息，如果使用`python run.py --no_onehot --model xgboost --new_preprocess`就可以看出它的效果。实验发现加不加onehot几乎无差别，说明对XGBoost来说可以不把年龄和性别转化为onehot信息。这也是它对比MLP的一个优势，MLP显然不能不加onehot地输进模型。
...
代码位于gnn.py和dataset.py。模型主要由两部分文件构成，`dataset.py`中的`TaobaoDataset`是一个[PyG](https://www.pyg.org/)风格的图数据集，在这个文件中我进行了原始数据的处理，并将其处理成一个可以直接用torch加载的`.pt`文件。原始文件需要手动放在`./data/taobao/raw`下，生成的文件将会储存在`./data/taobao/processed`下，具体如下:
...
我们采用了每个人主要负责一个模型和一个特征工程，其他同学辅助的形式。`第一种特征工程+随机森林+XGBoost`的主要负责人是夏箫，`第二种特征工程+MLP`的主要负责人是李伟楷，`图结构特征工+GraphSAGE`的主要负责人是李子昂。此外，李子昂和李伟楷负责了数据的统计分析。总体上三个人贡献相同。